...
class _confoundOutputSpec(TraitedSpec):
    confound_file = File(exists=True,
                         mandatory=True,
...
class ConfoundMatrix(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        cutoff = np.float(0.1)
        freqband = np.array([self.inputs.low_freq, self.inputs.high_freq
                             ]) / 60.
